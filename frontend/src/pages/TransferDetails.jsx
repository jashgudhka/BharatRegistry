...
const TRANSFER_STEPS = [
  ...
  { status: "completed", label: "Finalized", description: "Asset & funds distributed" },
];
    ...
    <div className="relative pt-6 pb-2">
      ...
      <div className="absolute left-[39px] top-6 bottom-6 w-0.5 bg-slate-100 z-0">
        <div 
          ...
          style={{ height: `${currentIndex > 0 ? (currentIndex / (TRANSFER_STEPS.length - 1)) * 100 : 0}%` }}
        ></div>
      </div>

      <div className="space-y-8 relative z-10">
        {TRANSFER_STEPS.map((step, index) => {
          const isCompleted = currentStatus === "completed" || index < currentIndex;
          const isCurrent = index === currentIndex && currentStatus !== "completed";
          const isPending = index > currentIndex;

          return (
            <div key={step.status} className={`flex items-start gap-5 group ${isPending ? 'opacity-50' : 'opacity-100'}`}>
              <div className="flex flex-col items-center">
                {isCompleted ? (
                   ...
                   <div className="w-10 h-10 rounded-full bg-white border-4 border-emerald-500 text-emerald-600 flex items-center justify-center shadow-[0_0_15px_rgba(16,185,129,0.3)] animate-pulse-glow">
                     ...
                     <span className="text-sm font-black relative z-10">{index + 1}</span>
                   </div>
                ) : (
              ...
              <div className="flex-1 pt-1.5 pb-2">
                <p
                  className={`font-extrabold tracking-tight ${
                    ...
                    : isCurrent ? "text-emerald-700 text-lg" 
                    : "text-slate-400"
                  }`}
                >
                  {step.label}
                </p>
                <p className={`text-sm mt-0.5 ${isCurrent ? 'text-emerald-600/80 font-medium' : 'text-slate-500'}`}>
                   {step.description}
                </p>
              </div>
            </div>
...
export default function TransferDetails() {
      ...
      <div className="max-w-3xl mx-auto w-full relative z-10 pt-12">
        <div className="glass-panel p-12 text-center border-dashed border-2 border-slate-200">
          ...
          <p className="text-slate-500 font-medium text-lg mb-10 max-w-md mx-auto">
            The requested transfer protocol could not be located on the distributed ledger.
          </p>
          <Link to="/transfers" className="btn btn-primary px-8 shadow-xl shadow-primary-500/20">
  ...
  const handleDepositEscrow = async () => {
    try {
      ...
      toast.success("Escrow funds secured mathematically!");
    } catch (err) {
  ...
  const handleApproveAsSeller = async () => {
    try {
      ...
      toast.success("Asset release verified!");
    } catch (err) {
  ...
  const handleApproveAsGovernment = async () => {
    try {
      ...
      toast.success("Consortium node consensus recorded!");
    } catch (err) {
  ...
  const handleComplete = async () => {
    try {
      ...
      toast.success("Protocol execution finalized!");
    } catch (err) {
  ...
  const handleCancel = async () => {
    if (!confirm("Are you certain you wish to abort this exchange protocol? Funds will be refunded.")) return;
    try {
      await cancelTransfer(transferId);
      toast.success("Protocol aborted successfully");
    } catch (err) {
  ...
  const statusColors = {
    pending: "text-amber-700 bg-amber-50 border-amber-200 shadow-[0_0_15px_rgba(251,191,36,0.15)]",
    deposited: "text-blue-700 bg-blue-50 border-blue-200 shadow-[0_0_15px_rgba(96,165,250,0.15)]",
    seller_approved: "text-indigo-700 bg-indigo-50 border-indigo-200 shadow-[0_0_15px_rgba(129,140,248,0.15)]",
    government_approved: "text-purple-700 bg-purple-50 border-purple-200 shadow-[0_0_15px_rgba(192,132,252,0.15)]",
    completed: "text-emerald-700 bg-emerald-50 border-emerald-200 shadow-[0_0_15px_rgba(52,211,153,0.15)]",
    cancelled: "text-red-700 bg-red-50 border-red-200 shadow-[0_0_15px_rgba(248,113,113,0.15)]",
    disputed: "text-red-700 bg-red-50 border-red-200 shadow-[0_0_15px_rgba(248,113,113,0.15)]",
  };
    ...
    <div className="max-w-6xl mx-auto space-y-8 animate-fade-in relative z-10 w-full pb-12">
      ...
      <div className="glass-panel p-8 sm:p-10 relative overflow-hidden shadow-lg border-b border-primary-100/50">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 relative z-10">
          <div>
            <div className="flex flex-wrap items-center gap-4 mb-3">
              ...
              <h1 className="text-4xl font-black text-slate-900 tracking-tight">
                Transfer <span className="text-primary-600 font-mono">#{transferId}</span>
              </h1>
              <span
          ...
          <div className="flex items-center gap-3 bg-white/60 p-3 pr-5 rounded-2xl border border-slate-200 shadow-sm whitespace-nowrap md:self-stretch items-center">
             ...
             <div>
                ...
                <p className="text-sm font-extrabold text-slate-700">Consortium Secured</p>
             </div>
          </div>
        </div>
      </div>
          ...
          <div className="glass-panel p-8 relative overflow-hidden">
            ...
            <div className="grid sm:grid-cols-2 gap-6">
              <div className="p-5 bg-white/60 border border-slate-200 hover:border-indigo-300 rounded-2xl shadow-sm hover:shadow-md transition-all group">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-xs font-bold uppercase tracking-widest text-slate-400 flex items-center gap-1.5">
                     <span className="w-1.5 h-1.5 rounded-full bg-indigo-400"></span> Current Owner
                  </span>
                  {isSeller && (
                    <span className="badge badge-verified bg-indigo-50 text-indigo-700 border-indigo-200 text-[10px] px-2 py-0.5">Your Identity</span>
                  )}
                </div>
              ...
              <div className="p-5 bg-white/60 border border-slate-200 hover:border-emerald-300 rounded-2xl shadow-sm hover:shadow-md transition-all group">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-xs font-bold uppercase tracking-widest text-slate-400 flex items-center gap-1.5">
                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-400"></span> Transferee
                  </span>
                  {isBuyer && (
                    <span className="badge badge-verified border-emerald-200 text-[10px] px-2 py-0.5">Your Identity</span>
                  )}
                </div>
          ...
          <div className="glass-panel p-8 relative overflow-hidden">
            ...
            <div className="grid sm:grid-cols-3 gap-6">
              <div className="p-6 bg-slate-50 border border-slate-200 rounded-2xl text-center shadow-sm relative group overflow-hidden">
                ...
                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">Agreed Valuation</p>
                <div className="flex items-baseline justify-center gap-1">
                  <span className="text-2xl font-black text-slate-800 tracking-tight">₹</span>
              ...
              <div className="p-6 bg-emerald-50 border border-emerald-200 rounded-2xl text-center shadow-sm relative group overflow-hidden">
                ...
                <p className="text-xs font-bold text-emerald-600/70 uppercase tracking-widest mb-2">Smart Escrow Lock</p>
                <div className="flex items-baseline justify-center gap-1">
                  <span className="text-2xl font-black text-emerald-700 tracking-tight">₹</span>
          ...
          <div className="glass-panel p-8 relative overflow-hidden border-t-4 border-t-blue-400">
            ...
            <div className="bg-white/50 rounded-2xl p-4 mb-6 border border-slate-100 shadow-inner">
               <div className="flex items-center justify-between text-sm">
                 <span className="text-slate-500 font-medium flex items-center gap-1.5"><Calendar size={14} /> Created:</span>
               ...
               {transfer.completedAt && (
                 <div className="flex items-center justify-between text-sm mt-2 pt-2 border-t border-slate-100">
                   <span className="text-green-600 font-medium flex items-center gap-1.5"><CheckCircle size={14} /> Finalized:</span>
                   <span className="font-bold text-slate-800 tracking-tight">
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-emerald-700/80 leading-relaxed">
                    The digital asset representation has been immutably transferred to the new owner. Financial settlement is finalized.
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-red-700/80 leading-relaxed">
                    This transfer sequence was intentionally halted. All locked escrow funds have been autonomously returned via smart contract logic.
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="glass-panel p-6 border-t-4 border-t-primary-500 shadow-[0_20px_40px_-15px_rgba(84,129,255,0.2)] bg-gradient-to-br from-white to-primary-50/30">
                ...
                <div className="space-y-4">
                  ...
                  {isBuyer && transfer.status === "pending" && (
                    ...
                    >
                      ...
                      <span className="relative z-10 flex items-center justify-center gap-2">
                        {depositing ? (
                          <><svg className="animate-spin h-5 w-5" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" /></svg> Securing Funds...</>
                        ) : "Secure Escrow Funds"}
                      </span>
                    </button>
                  )}
                  ...
                  {transfer.status === "seller_approved" && (
                    ...
                    >
                      {approvingGov ? "Validating..." : "Execute Consensus Validation"}
                    </button>
                  )}
                  ...
                  {transfer.status === "government_approved" && (
                    ...
                    >
                      {completing ? "Finalizing Ledger..." : "Finalize Protocol Execution"}
                    </button>
                  )}
                  ...
                  {!((isBuyer && transfer.status === "pending") || 
                    ...
                    <div className="text-center py-4 bg-white/50 rounded-xl border border-slate-100">
                       ...
                       <p className="text-sm font-bold text-slate-500">Waiting for counterparty action...</p>
                    </div>
                  )}
                </div>
              </div>
